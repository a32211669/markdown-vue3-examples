...
import {
  ...
  b as getPointResolution,
  bt as intersects,
  ct as getArea,
  dt as getCenter,
  f as createTransformFromCoordinateTransform,
  ht as getIntersection,
  it as extend,
  lt as getBottomLeft,
  mt as getHeight,
  st as forEachCorner,
  ut as getBottomRight,
  vt as getTopRight,
  wt as wrapAndSliceX,
  x as getTransform,
  yt as getWidth,
  ...
  constructor(
    ...
    const transformInv = sourceMatrix
      ? createTransformFromCoordinateTransform((input) =>
          apply(sourceMatrix, transform(input, this.targetProj_, this.sourceProj_)),
        )
    ...
    const maxSubdivision =
      ...
      (destinationResolution
        ? Math.max(
            0,
            Math.ceil(Math.log2(getArea(targetExtent) / (destinationResolution * destinationResolution * 256 * 256))),
          )
        : 0);
    this.addQuad_(
      destinationTopLeft,
    ...
    if (this.wrapsXInSource_) {
      ...
      this.triangles_.forEach((triangle) => {
        if (
          Math.max(triangle.source[0][0], triangle.source[1][0], triangle.source[2][0]) - leftBound >
          ...
          if (Math.max(newTriangle[0][0], newTriangle[1][0], newTriangle[2][0]) - minX < this.sourceWorldWidth_ / 2)
  ...
  addQuad_(a, b, c, d, aSrc, bSrc, cSrc, dSrc, maxSubdivision) {
    ...
    if (maxSubdivision > 0) {
      if (this.targetProj_.isGlobal() && this.targetWorldWidth_)
        needsSubdivision =
          getWidth(boundingExtent([a, b, c, d])) / this.targetWorldWidth_ > MAX_TRIANGLE_WIDTH || needsSubdivision;
    ...
    if (maxSubdivision > 0) {
      if (!needsSubdivision) {
        ...
        if (wrapsX)
          dx =
            (modulo(aSrc[0], sourceWorldWidth) + modulo(cSrc[0], sourceWorldWidth)) / 2 -
...
var ReprojTile = class extends Tile {
  ...
  constructor(
    ...
    this.triangulation_ = new Triangulation(
      ...
      limitedTargetExtent,
      maxSourceExtent,
      sourceResolution * (errorThreshold !== void 0 ? errorThreshold : ERROR_THRESHOLD),
      targetResolution,
    );
      ...
      this.sourceTiles_.forEach(({ tile }) => {
        ...
        if (state == TileState_default.IDLE || state == TileState_default.LOADING) {
          ...
          const sourceListenKey = listen(tile, EventType_default.CHANGE, (e) => {
            const state = tile.getState();
            if (
              state == TileState_default.LOADED ||
              state == TileState_default.ERROR ||
              state == TileState_default.EMPTY
            ) {
...
export {
  ReprojTile as a,
  ImageTile as c,
  withinExtentAndZ as i,
  Tile as l,
  getCacheKey as n,
  TileRange as o,
  hash as r,
  createOrUpdate$1 as s,
  createOrUpdate as t,
};
