...
var CanvasTileLayerRenderer = class extends CanvasLayerRenderer {
  ...
  enqueueTiles(frameState, extent, initialZ, tilesByZ, preload) {
    ...
    const viewport = rotation
      ? getRotatedViewport(viewState.center, viewState.resolution, rotation, frameState.size)
      : void 0;
    for (let z = initialZ; z >= minZ; --z) {
      ...
      for (let x = tileRange.minX; x <= tileRange.maxX; ++x)
        for (let y = tileRange.minY; y <= tileRange.maxY; ++y) {
          if (rotation && !tileGrid.tileCoordIntersectsViewport([z, x, y], viewport)) continue;
  ...
  renderFrame(frameState, target) {
    ...
    for (const tile of tilesByZ[z]) {
      ...
      for (let parentZ = z - 1; parentZ >= minZoom; --parentZ)
        if (this.findAltTiles_(tileGrid, tileCoord, parentZ, tilesByZ)) break;
    }
    /**
