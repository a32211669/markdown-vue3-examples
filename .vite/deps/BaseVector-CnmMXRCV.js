...
function hasArgsCount(minArgs, maxArgs) {
  return function (encoded, returnType, context) {
    ...
    if (minArgs === maxArgs) {
      if (argCount !== minArgs)
        throw new Error(`expected ${minArgs} argument${minArgs === 1 ? '' : 's'} for ${operation}, got ${argCount}`);
...
function interpolateNumber(base, value, input1, output1, input2, output2) {
  ...
  return (
    output1 +
    (base === 1 ? along / delta : (Math.pow(base, along) - 1) / (Math.pow(base, delta) - 1)) * (output2 - output1)
  );
}
/**
 * @param {number} base The base.
...
function buildIcon(flatStyle, context) {
  ...
  if (colorValue !== void 0)
    if (Array.isArray(colorValue) && colorValue.length > 0 && typeof colorValue[0] === 'string')
