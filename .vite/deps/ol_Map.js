import {
  E as ObjectEventType_default,
  T as EventType_default$1,
  c as unlistenByKey,
  d as FALSE,
  f as TRUE,
  h as toPromise,
...
import {
  B as scale,
  D as toUserCoordinate,
  H as warn,
  K as clone,
  St as isEmpty,
  V as wrapX,
  et as createOrUpdateEmpty,
  g as fromUserCoordinate,
  ht as getIntersection,
  pt as getForViewAndSize,
  rt as equals$1,
  yt as getWidth,
  z as rotate,
} from './proj-BYDU1skc.js';
import { n as clamp } from './math-BYtIDyQ8.js';
import { a as compose, c as fromString, i as apply, l as makeInverse, o as create } from './contains-Bl5givOM.js';
import { t as Polygon } from './Polygon-CwOeeBbl.js';
import { t as TileState_default } from './TileState-BV9P4F7q.js';
import { i as linear, n as easeOut } from './easing-Ds8nYaIx.js';
import {
  _ as PASSIVE_EVENT_LISTENERS,
  c as createMockDiv,
  f as removeChildren,
  g as MAC,
  h as DEVICE_PIXEL_RATIO,
  m as replaceNode,
  p as replaceChildren,
  t as hasArea,
  u as isCanvas,
  v as WEBKIT,
...
import {
  a as RenderEvent,
  i as EventType_default$2,
  n as inView,
  r as BaseLayer,
  t as Layer,
} from './Layer-CkA48-Ee.js';
import { d as disable, f as ViewHint_default, t as View } from './View-CUp4qALX.js';
import {
  D as CLASS_HIDDEN,
  E as CLASS_CONTROL,
  O as CLASS_UNSELECTABLE,
  T as CLASS_COLLAPSED,
  d as checkedFonts,
  j as shared,
} from './Text-Dob7Y1Fx.js';
import { t as BaseVectorLayer } from './BaseVector-CnmMXRCV.js';
import { n as CollectionEventType_default, t as Collection } from './Collection-DwxDWj5o.js';
...
var MapBrowserEventHandler = class extends Target {
  ...
  updateActivePointers_(pointerEvent) {
    ...
    if (
      event.type == MapBrowserEventType_default.POINTERUP ||
      event.type == MapBrowserEventType_default.POINTERCANCEL
    ) {
      ...
      event.type == MapBrowserEventType_default.POINTERDOWN ||
      event.type == MapBrowserEventType_default.POINTERMOVE
    )
  ...
  handlePointerDown_(pointerEvent) {
    ...
    if (this.dragListenerKeys_.length === 0) {
      ...
      if (this.element_.getRootNode && this.element_.getRootNode() !== doc)
        this.dragListenerKeys_.push(
          listen(this.element_.getRootNode(), MapBrowserEventType_default.POINTERUP, this.handlePointerUp_, this),
        );
    }
  }
  /**
  ...
  handleTileChange(event) {
    ...
    if (state === TileState_default.LOADED || state === TileState_default.ERROR || state === TileState_default.EMPTY) {
      if (state !== TileState_default.ERROR)
        tile.removeEventListener(EventType_default$1.CHANGE, this.boundHandleTileChange_);
...
var Attribution = class extends Control {
  ...
  constructor(options) {
    ...
    const collapseClassName =
      options.collapseClassName !== void 0 ? options.collapseClassName : className + '-collapse';
  ...
  async updateElement_(frameState) {
    ...
    const attributions = await Promise.all(
      this.collectSourceAttributions_(frameState).map((attribution) => toPromise(() => attribution)),
    );
...
function defaults$1(options) {
  ...
  if (options.attribution !== void 0 ? options.attribution : true)
...
var focusWithTabindex = function (event) {
  const targetElement = event.map.getTargetElement();
  const rootNode = targetElement.getRootNode();
  return (rootNode instanceof ShadowRoot ? rootNode.host : targetElement).hasAttribute('tabindex')
    ? focus(event)
    : true;
};
/**
...
var targetNotEditable = function (mapBrowserEvent) {
  ...
  return (
    tagName !== 'INPUT' && tagName !== 'SELECT' && tagName !== 'TEXTAREA' && !originalEvent.target.isContentEditable
  );
};
/**
  ...
  handleEvent(mapBrowserEvent) {
    ...
    if (this.handlingDownUpSequence) {
      if (mapBrowserEvent.type == MapBrowserEventType_default.POINTERDRAG) {
        ...
        this.handlingDownUpSequence = this.handleUpEvent(mapBrowserEvent) && this.targetPointers.length > 0;
...
var RenderBox = class extends Disposable {
  ...
  createOrUpdateGeometry() {
    ...
    const startPixel = this.startPixel_;
    const endPixel = this.endPixel_;
    const coordinates = [startPixel, [startPixel[0], endPixel[1]], endPixel, [endPixel[0], startPixel[1]]].map(
  ...
  handleEvent(mapBrowserEvent) {
    ...
    if (mapBrowserEvent.type == EventType_default$1.KEYDOWN) {
      ...
      if (
        ...
        (key == Key_default.DOWN || key == Key_default.LEFT || key == Key_default.RIGHT || key == Key_default.UP)
      ) {
        const view = mapBrowserEvent.map.getView();
...
var MouseWheelZoom = class extends Interaction {
  ...
  handleWheelZoom_(map) {
    ...
    if (view.getAnimating()) view.cancelAnimations();
    let delta =
      -clamp(this.totalDelta_, -this.maxDelta_ * this.deltaPerZoom_, this.maxDelta_ * this.deltaPerZoom_) /
      this.deltaPerZoom_;
...
function defaults(options) {
  ...
  if (options.doubleClickZoom !== void 0 ? options.doubleClickZoom : true)
    interactions.push(
      new DoubleClickZoom({
  ...
  if (options.dragPan !== void 0 ? options.dragPan : true)
    interactions.push(
      new DragPan({
  ...
  if (options.pinchZoom !== void 0 ? options.pinchZoom : true)
    interactions.push(new PinchZoom({ duration: options.zoomDuration }));
  ...
  if (options.mouseWheelZoom !== void 0 ? options.mouseWheelZoom : true)
    interactions.push(
      new MouseWheelZoom({
        onFocusOnly: options.onFocusOnly,
        duration: options.zoomDuration,
  ...
  if (options.shiftDragZoom !== void 0 ? options.shiftDragZoom : true)
    interactions.push(new DragZoom({ duration: options.zoomDuration }));
  return interactions;
}
//#endregion
...
var LayerGroup = class LayerGroup extends BaseLayer {
  ...
  constructor(options) {
    ...
    if (layers)
      if (Array.isArray(layers)) layers = new Collection(layers.slice(), { unique: true });
  ...
  registerLayerListeners_(layer) {
    const listenerKeys = [
      ...
      listen(layer, EventType_default$1.CHANGE, this.handleLayerChange_, this),
    ];
    if (layer instanceof LayerGroup)
      listenerKeys.push(
        listen(layer, GroupEventType.ADDLAYER, this.handleLayerGroupAdd_, this),
        listen(layer, GroupEventType.REMOVELAYER, this.handleLayerGroupRemove_, this),
      );
  ...
  getLayerStatesArray(dest) {
    ...
    for (let i = pos, ii = states.length; i < ii; i++) {
      ...
      if (ownLayerState.extent !== void 0)
        if (layerState.extent !== void 0) layerState.extent = getIntersection(layerState.extent, ownLayerState.extent);
...
var Map = class extends BaseObject {
  ...
  constructor(options) {
     ...
     */
    this.interactions =
      optionsInternal.interactions || (WORKER_OFFSCREEN_CANVAS ? new Collection() : defaults({ onFocusOnly: true }));
    /**
  ...
  handleMapBrowserEvent(mapBrowserEvent) {
    ...
    if (
      eventType === EventType_default.POINTERDOWN ||
      eventType === EventType_default$1.WHEEL ||
      eventType === EventType_default$1.KEYDOWN
    ) {
  ...
  handlePostRender() {
    ...
    if (frameState && this.renderer_ && !frameState.animate) {
      if (this.renderComplete_) {
        if (this.hasListener(EventType_default$2.RENDERCOMPLETE))
          this.renderer_.dispatchRenderEvent(EventType_default$2.RENDERCOMPLETE, frameState);
    ...
    if (!targetElement) {
      ...
      if (!isCanvas(targetElement)) {
        ...
        this.targetChangeHandlerKeys_ = [
          listen(keyboardEventTarget, EventType_default$1.KEYDOWN, this.handleBrowserEvent, this),
          listen(keyboardEventTarget, EventType_default$1.KEYPRESS, this.handleBrowserEvent, this),
        ];
  ...
  renderFrame_(time) {
    ...
    if (frameState) {
      ...
      if (previousFrameState) {
        if (
          ...
          (!isEmpty(this.previousExtent_) && !equals$1(frameState.extent, this.previousExtent_))
        ) {
      ...
      if (
        this.previousExtent_ &&
        !frameState.viewHints[ViewHint_default.ANIMATING] &&
        !frameState.viewHints[ViewHint_default.INTERACTING] &&
    ...
    this.renderComplete_ =
      (this.hasListener(MapEventType_default.LOADSTART) ||
        this.hasListener(MapEventType_default.LOADEND) ||
        this.hasListener(EventType_default$2.RENDERCOMPLETE)) &&
...
function createOptionsInternal(options) {
  ...
  const values = {};
  const layerGroup =
    options.layers && typeof options.layers.getLayers === 'function'
      ? options.layers
      : new LayerGroup({ layers: options.layers });
  ...
  if (options.controls !== void 0)
    if (Array.isArray(options.controls)) controls = new Collection(options.controls.slice());
    else {
      assert(
  ...
  if (options.interactions !== void 0)
    if (Array.isArray(options.interactions)) interactions = new Collection(options.interactions.slice());
    else {
      assert(
  ...
  if (options.overlays !== void 0)
    if (Array.isArray(options.overlays)) overlays = new Collection(options.overlays.slice());
    else {
      assert(
