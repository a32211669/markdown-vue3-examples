    ...
    for (; offset < str.length; offset++, this.excess++) {
      ...
      if (this.treeIndex < 0)
        return this.result === 0 ||
          (this.decodeMode === DecodingMode.Attribute && (valueLength === 0 || isEntityInAttributeInvalidEnd(char)))
          ? 0
...
//#endregion
//#region node_modules/markdown-it/node_modules/entities/lib/esm/index.js
...
function strikethrough_postProcess(state) {
  ...
  for (let curr = 0; curr < max; curr++)
    if (tokens_meta[curr] && tokens_meta[curr].delimiters) postProcess$1(state, tokens_meta[curr].delimiters);
}
...
function emphasis_post_process(state) {
  ...
  for (let curr = 0; curr < max; curr++)
    if (tokens_meta[curr] && tokens_meta[curr].delimiters) postProcess(state, tokens_meta[curr].delimiters);
}
...
function link_pairs(state) {
  ...
  for (let curr = 0; curr < max; curr++)
    if (tokens_meta[curr] && tokens_meta[curr].delimiters) processDelimiters(tokens_meta[curr].delimiters);
}
//#endregion
