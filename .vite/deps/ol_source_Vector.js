...
import {
  a as getInteriorPointsOfMultiArray,
  c as quantizeArray,
  f as transform2D,
  i as getInteriorPointOfArray,
  o as douglasPeucker,
  s as douglasPeuckerArray,
  t as inflateEnds,
...
var RBush = class {
  ...
  update(extent, value) {
    ...
    if (!equals([item.minX, item.minY, item.maxX, item.maxY], extent)) {
      this.remove(value);
...
var VectorSource = class extends Source {
    ...
    this.featuresRtree_.forEachInExtent(
      ...
      function (feature) {
        if (filter(feature)) {
          ...
          minSquaredDistance =
            geometry instanceof RenderFeature ? 0 : geometry.closestPointXY(x, y, closestPoint, minSquaredDistance);
  ...
  loadFeatures(extent, resolution, projection) {
    ...
    for (let i = 0, ii = extentsToLoad.length; i < ii; ++i) {
      ...
      if (
        !loadedExtentsRtree.forEachInExtent(
          extentToLoad,
          /**
