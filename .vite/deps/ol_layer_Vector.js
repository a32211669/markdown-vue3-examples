...
import { $ as createOrUpdate, C as getUserProjection, Et as Relationship_default, G as buffer, J as containsCoordinate, K as clone, L as angleBetween, O as toUserExtent, Q as createEmpty, S as getTransformFromProjections, St as isEmpty$1, Tt as wrapX$1, V as wrapX, Y as containsExtent, Z as coordinateRelationship, _ as fromUserExtent, at as extendCoordinate, bt as intersects, k as toUserResolution, mt as getHeight, yt as getWidth } from "./proj-BYDU1skc.js";
...
import { c as inflateMultiCoordinatesArray, l as transformGeom2D, o as inflateCoordinates, s as inflateCoordinatesArray } from "./Polygon-CwOeeBbl.js";
...
import { C as measureAndCacheTextWidth, S as getTextDimensions, _ as defaultPadding, b as defaultTextBaseline, f as defaultFillStyle, g as defaultLineJoin, h as defaultLineDash, k as asColorLike, m as defaultLineCap, o as Icon, p as defaultFont, v as defaultStrokeStyle, w as registerFont, x as drawImageOrLabel, y as defaultTextAlign } from "./Text-Dob7Y1Fx.js";
...
var Executor = class {
	...
	*/
	execute_(context, scaledCanvasSize, transform, instructions, snapToPixel, featureCallback, hitExtent, declutterTree) {
		...
		let d = 0;
		let dd;
		let anchorX, anchorY, lineOffsetPx, isClosedRing, dStart, declutterMode, prevX, prevY, roundX, roundY, image, text, textKey, strokeKey, fillKey;
		let pendingFill = 0;
		let pendingStroke = 0;
		...
		while (i < ii) {
			...
			switch (instruction[0]) {
				...
				case Instruction.MOVE_TO_LINE_TO:
					...
					if (lineOffsetPx) {
						dStart = d;
						[x, y] = offsetLineVertex(x, y, isClosedRing ? pixelCoordinates[dd - 4] : void 0, isClosedRing ? pixelCoordinates[dd - 3] : void 0, pixelCoordinates[d + 2], pixelCoordinates[d + 3], lineOffsetPx);
					}
					context.moveTo(x, y);
					prevX = x + .5 | 0;
					prevY = y + .5 | 0;
					for (d += 2; d < dd; d += 2) {
						...
						if (d == dd - 2 || roundX !== prevX || roundY !== prevY) {
							if (lineOffsetPx) if (d == dd - 2) [x, y] = offsetLineVertex(x, y, pixelCoordinates[d - 2], pixelCoordinates[d - 1], isClosedRing ? pixelCoordinates[dStart + 2] : void 0, isClosedRing ? pixelCoordinates[dStart + 3] : void 0, lineOffsetPx);
							else [x, y] = offsetLineVertex(x, y, pixelCoordinates[d - 2], pixelCoordinates[d - 1], pixelCoordinates[d + 2], pixelCoordinates[d + 3], lineOffsetPx);
							context.lineTo(x, y);
							prevX = roundX;
							prevY = roundY;
						}
...
var CanvasImmediateRenderer = class extends VectorContext {
	...
	*/
	moveToLineTo_(flatCoordinates, offset, end, stride, close, strokeOffset) {
		...
		if (Math.abs(strokeOffset) > 0) pixelCoordinates = offsetLineString(pixelCoordinates, stride, strokeOffset, close, pixelCoordinates);
