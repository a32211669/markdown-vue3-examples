import { C as reverseSubArray, g as ascending, i as getUid, v as descending, y as equals } from './asserts-DUNxUKpS.js';
import { n as isEmpty } from './obj-C0zdAnF3.js';
import {
  $ as createOrUpdate,
  C as getUserProjection,
  Et as Relationship_default,
  G as buffer,
  J as containsCoordinate,
  K as clone,
  L as angleBetween,
  O as toUserExtent,
  Q as createEmpty,
  S as getTransformFromProjections,
  St as isEmpty$1,
  Tt as wrapX$1,
  V as wrapX,
  Y as containsExtent,
  Z as coordinateRelationship,
  _ as fromUserExtent,
  at as extendCoordinate,
  bt as intersects,
  k as toUserResolution,
  mt as getHeight,
  yt as getWidth,
} from './proj-BYDU1skc.js';
import { i as lerp, n as clamp, u as toFixed } from './math-BYtIDyQ8.js';
import { a as compose, i as apply, o as create, u as setFromArray } from './contains-Bl5givOM.js';
import { f as transform2D, l as snap, u as rotate } from './orient-BQDaEeHr.js';
import {
  c as inflateMultiCoordinatesArray,
  l as transformGeom2D,
  o as inflateCoordinates,
  s as inflateCoordinatesArray,
} from './Polygon-CwOeeBbl.js';
import { d as releaseCanvas, o as ImageState_default, s as createCanvasContext2D } from './size-DfTGL_qp.js';
import { i as EventType_default } from './Layer-CkA48-Ee.js';
import { n as canvasPool, r as ZIndexContext, t as CanvasLayerRenderer } from './Layer-CotuPRNK.js';
import { f as ViewHint_default } from './View-CUp4qALX.js';
import {
  C as measureAndCacheTextWidth,
  S as getTextDimensions,
  _ as defaultPadding,
  b as defaultTextBaseline,
  f as defaultFillStyle,
  g as defaultLineJoin,
  h as defaultLineDash,
  k as asColorLike,
  m as defaultLineCap,
  o as Icon,
  p as defaultFont,
  v as defaultStrokeStyle,
  w as registerFont,
  x as drawImageOrLabel,
  y as defaultTextAlign,
...
var CanvasBuilder = class extends VectorContext {
  ...
  drawCustom(geometry, feature, renderer, hitDetectionRenderer, index) {
    ...
    switch (type) {
      ...
      case 'Point':
        ...
        this.instructions.push([Instruction.CUSTOM, builderBegin, builderEnd, geometry, renderer, void 0, index]);
        this.hitDetectionInstructions.push([
          Instruction.CUSTOM,
          builderBegin,
          builderEnd,
  ...
  beginGeometry(geometry, feature, index) {
    this.beginGeometryInstruction1_ = [Instruction.BEGIN_GEOMETRY, feature, 0, geometry, index];
    this.instructions.push(this.beginGeometryInstruction1_);
    this.beginGeometryInstruction2_ = [Instruction.BEGIN_GEOMETRY, feature, 0, geometry, index];
  ...
  fillStyleToState(fillStyle, state = {}) {
    if (fillStyle) {
      ...
      state.fillPatternScale =
        fillStyleColor && typeof fillStyleColor === 'object' && 'src' in fillStyleColor ? this.pixelRatio : 1;
...
var CanvasLineStringBuilder = class extends CanvasBuilder {
  ...
  drawFlatCoordinates_(flatCoordinates, offset, end, stride, strokeOffset) {
    ...
    this.instructions.push([Instruction.MOVE_TO_LINE_TO, myBegin, myEnd, strokeOffset * this.pixelRatio]);
    this.hitDetectionInstructions.push([Instruction.MOVE_TO_LINE_TO, myBegin, myEnd, strokeOffset]);
    return end;
  }
  /**
   * @param {import("../../geom/LineString.js").default|import("../Feature.js").default} lineStringGeometry Line string geometry.
  ...
  drawMultiLineString(multiLineStringGeometry, feature, index) {
    ...
    let offset = 0;
    for (let i = 0, ii = ends.length; i < ii; ++i)
      offset = this.drawFlatCoordinates_(flatCoordinates, offset, ends[i], stride, strokeOffset);
    this.hitDetectionInstructions.push(strokeInstruction);
    this.endGeometry(feature);
  }
  /**
...
var CanvasPolygonBuilder = class extends CanvasBuilder {
  ...
  drawFlatCoordinatess_(flatCoordinates, offset, ends, stride, strokeOffset) {
    ...
    for (let i = 0; i < numEnds; ++i) {
      ...
      this.instructions.push([Instruction.MOVE_TO_LINE_TO, myBegin, myEnd, strokeOffset * this.pixelRatio, true]);
      this.hitDetectionInstructions.push([Instruction.MOVE_TO_LINE_TO, myBegin, myEnd, strokeOffset, true]);
      if (stroke) {
  ...
  drawMultiPolygon(multiPolygonGeometry, feature, index) {
    ...
    let offset = 0;
    for (let i = 0, ii = endss.length; i < ii; ++i)
      offset = this.drawFlatCoordinatess_(flatCoordinates, offset, endss[i], stride, strokeOffset);
    this.endGeometry(feature);
  }
  /**
   * @return {import("../canvas.js").SerializableInstructions} the serializable instructions.
 ...
 */
function offsetLineString(flatCoordinates, stride, offset, isClosedRing, dest, destinationStride) {
  dest = dest ?? [];
...
function offsetLineVertex(x, y, prevX, prevY, nextX, nextY, offset) {
  ...
  if (Math.cos(joinAngle) > 0.998) return [x + tx * offset, y + ty * offset];
...
function drawTextOnPath(
  ...
  if (singleSegment) {
    ...
    result[0] = [(endX + beginX) / 2, (endY + beginY) / 2, (endM - startM) / 2, previousAngle, text];
    return result;
  }
  text = text.replace(/\n/g, ' ');
...
var Executor = class {
  ...
  createLabel(text, textKey, fillKey, strokeKey) {
    ...
    const align = textState.justify
      ...
      : horizontalTextAlign(Array.isArray(text) ? text[0] : text, textState.textAlign || 'center');
  ...
  drawLabelWithPointPlacement_(text, textKey, strokeKey, fillKey) {
    ...
    return {
      label,
      anchorX: align * (label.width / pixelRatio - 2 * textState.scale[0]) + 2 * (0.5 - align) * strokeWidth,
      anchorY: (baseline * label.height) / pixelRatio + 2 * (0.5 - baseline) * strokeWidth,
    };
  }
  /**
   * @private
    ...
    while (i < ii) {
      ...
      switch (instruction[0]) {
              ...
              if (strokeKey)
                for (c = 0, cc = parts.length; c < cc; ++c) {
                  ...
                  anchorY =
                    ...
                    ((0.5 - baseline) * 2 * strokeWidth * textScale[1]) / textScale[0] -
                    offsetY;
                  const dimensions = this.calculateImageOrLabelDimensions_(
                    label.width,
                    label.height,
        ...
        case Instruction.MOVE_TO_LINE_TO:
          ...
          isClosedRing = instruction[4] ?? false;
          ...
          if (lineOffsetPx) {
            ...
            [x, y] = offsetLineVertex(
              x,
              y,
              isClosedRing ? pixelCoordinates[dd - 4] : void 0,
              isClosedRing ? pixelCoordinates[dd - 3] : void 0,
              ...
              if (lineOffsetPx)
                if (d == dd - 2)
                  [x, y] = offsetLineVertex(
                    ...
                    pixelCoordinates[d - 2],
                    pixelCoordinates[d - 1],
                    isClosedRing ? pixelCoordinates[dStart + 2] : void 0,
                    isClosedRing ? pixelCoordinates[dStart + 3] : void 0,
                    lineOffsetPx,
                  );
                else
                  [x, y] = offsetLineVertex(
...
var ExecutorGroup = class {
  ...
  forEachFeatureAtCoordinate(coordinate, resolution, rotation, hitTolerance, callback, declutteredFeatures) {
    ...
    if (newContext)
      this.hitDetectionContext_ = createCanvasContext2D(contextSize, contextSize, null, { willReadFrequently });
...
var CanvasImmediateRenderer = class extends VectorContext {
  ...
  moveToLineTo_(flatCoordinates, offset, end, stride, close, strokeOffset) {
    ...
    if (Math.abs(strokeOffset) > 0)
      pixelCoordinates = offsetLineString(pixelCoordinates, stride, strokeOffset, close, pixelCoordinates);
  ...
  drawRings_(flatCoordinates, offset, ends, stride, strokeOffset) {
    for (let i = 0, ii = ends.length; i < ii; ++i)
      offset = this.moveToLineTo_(flatCoordinates, offset, ends[i], stride, true, strokeOffset);
    return offset;
  }
  /**
  ...
  drawMultiLineString(geometry) {
    ...
    if (this.strokeState_) {
      ...
      context.beginPath();
      for (let i = 0, ii = ends.length; i < ii; ++i)
        offset = this.moveToLineTo_(flatCoordinates, offset, ends[i], stride, false, this.strokeState_.strokeOffset);
      context.stroke();
    }
    if (this.text_ !== '') {
...
function renderGeometry(replayGroup, geometry, style, feature, index) {
  ...
  replayGroup
    .getBuilder(style.getZIndex(), 'Default')
    .drawCustom(geometry, feature, style.getRenderer(), style.getHitDetectionRenderer(), index);
}
/**
 * @param {import("../render/canvas/BuilderGroup.js").default} replayGroup Replay group.
...
var CanvasVectorLayerRenderer = class extends CanvasLayerRenderer {
  ...
  renderWorlds(executorGroup, frameState, declutterable) {
    ...
    const endWorld = multiWorld
      ? Math.ceil((extent[2] - projectionExtent[2]) / worldWidth) + (this.extendX_ ? 2 : 1)
      : 1;
  ...
  renderFrame(frameState, target) {
    ...
    if (!render) {
      if (
        !(
          this.getLayer().hasListener(EventType_default.PRERENDER) ||
          this.getLayer().hasListener(EventType_default.POSTRENDER)
        )
      )
    ...
    if (render && layerState.extent && this.clipping) {
      ...
      if (render && !containsExtent(layerExtent, frameState.extent))
        if (frameState.declutter) this.clipExtent_ = layerExtent;
        else {
  ...
  getFeatures(pixel) {
    return new Promise((resolve) => {
      if (this.frameState && !this.hitDetectionImageData_ && !this.animatingOrInteracting_) {
        ...
        transforms.push(
          this.getRenderTransform(center, resolution, rotation, HIT_DETECT_RESOLUTION, width, height, 0).slice(),
        );
  ...
  prepareFrame(frameState) {
    ...
    if (canWrapX) {
      ...
      if (sourceExtent && !isEmpty$1(sourceExtent))
