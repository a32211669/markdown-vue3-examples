...
function isRouteComponent(component) {
  return (
    typeof component === 'object' || 'displayName' in component || 'props' in component || '__vccOpts' in component
  );
}
...
function isSameRouteLocation(stringifyQuery$1, a, b) {
  ...
  return (
    ...
    stringifyQuery$1(a.query) === stringifyQuery$1(b.query) &&
    a.hash === b.hash
  );
}
/**
...
function stringifyQuery(query) {
  ...
  for (let key in query) {
    ...
    (isArray(value) ? value.map((v) => v && encodeQueryValue(v)) : [value && encodeQueryValue(value)]).forEach(
      (value$1) => {
        if (value$1 !== void 0) {
  ...
  return () =>
    new Promise((resolve, reject) => {
      ...
      const guardReturn = runWithContext(() =>
        guard.call(record && record.instances[name], to, from, canOnlyBeCalledOnce(next, to, from)),
      );
  ...
  for (const record of matched) {
    ...
    for (const name in record.components) {
      ...
      if (!rawComponent || (typeof rawComponent !== 'object' && typeof rawComponent !== 'function')) {
        warn$1(
          `Component "${name}" in record with path "${record.path}" is not a valid component. Received "${String(
            rawComponent,
          )}".`,
        );
        ...
        warn$1(
          `Component "${name}" in record with path "${record.path}" is defined using "defineAsyncComponent()". Write "() => import('./MyPage.vue')" instead of "defineAsyncComponent(() => import('./MyPage.vue'))".`,
        );
      }
...
function loadRouteLocation(route) {
  return route.matched.every((record) => record.redirect)
    ? Promise.reject(/* @__PURE__ */ new Error('Cannot load a route that redirects.'))
    : Promise.all(
        route.matched.map(
          (record) =>
            record.components &&
...
function useHistoryStateNavigation(base) {
  ...
  function changeLocation(to, state, replace$1) {
    ...
    const url =
      hashIndex > -1
        ? (location$1.host && document.querySelector('base') ? base : base.slice(hashIndex)) + to
  ...
  function replace(to, data) {
    changeLocation(
      to,
      assign({}, history$1.state, buildState(historyState.value.back, to, historyState.value.forward, true), data, {
  ...
  function push(to, data) {
    ...
    changeLocation(
      to,
      assign({}, buildState(currentLocation.value, to, null), { position: currentState.position + 1 }, data),
      false,
    );
    currentLocation.value = to;
  }
  ...
  function stringify(params) {
    ...
    for (const segment of segments) {
      ...
      for (const token of segment)
        if (token.type === TokenType.Static) path += token.value;
        else if (token.type === TokenType.Param) {
...
function createRouterMatcher(routes, globalOptions) {
  ...
  function resolve(location$1, currentLocation) {
    ...
    if ('name' in location$1 && location$1.name) {
      ...
      {
        const invalidParams = Object.keys(location$1.params || {}).filter(
...
function includesParams(outer, inner) {
  for (const key in inner) {
    ...
    if (typeof innerValue === 'string') {
      ...
      innerValue.some((value, i) => value.valueOf() !== outerValue[i].valueOf())
    )
      return false;
  }
  return true;
...
function createRouter(options) {
  ...
  if (!routerHistory)
    throw new Error(
      'Provide the "history" option when calling "createRouter()": https://router.vuejs.org/api/interfaces/RouterOptions.html#history',
    );
  ...
  function resolve(rawLocation, currentLocation) {
    ...
    if (typeof rawLocation === 'string') {
      ...
      if (href$1.startsWith('//'))
        warn$1(
          `Location "${rawLocation}" resolved to "${href$1}". A resolved location cannot start with multiple slashes.`,
        );
      ...
      .then((failure$1) => {
        if (failure$1) {
          if (isNavigationFailure(failure$1, ErrorTypes.NAVIGATION_GUARD_REDIRECT)) {
            if (
              isSameRouteLocation(stringifyQuery$1, resolve(failure$1.to), toLocation) &&
              redirectedFrom &&
      ...
      navigate(toLocation, from)
        .catch((error) => {
          ...
          if (isNavigationFailure(error, ErrorTypes.NAVIGATION_GUARD_REDIRECT)) {
            pushWithRedirect(assign(locationAsObject(error.to), { force: true }), toLocation)
              .then((failure) => {
                if (
